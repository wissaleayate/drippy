...
export default function ProductGrid() {
  ...
  return (
    <section className="bg-ink py-16 sm:py-20 md:py-32">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 sm:gap-6 mb-8 sm:mb-12">
          <div>
            ...
            <h2 className="font-display font-black text-bone leading-none" style={{ fontSize: 'clamp(32px, 5vw, 72px)', letterSpacing: '-0.02em' }}>{t.grid_heading}</h2>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map((c) => (
              <button
                ...
                className="tap-target px-3 sm:px-4 py-2 text-xs tracking-[0.12em] uppercase transition-all duration-200"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
          {filtered.map((p) => (
            ...
            >
              ...
              <div className="overlay absolute inset-0 flex flex-col justify-end p-4 sm:p-5" style={{ background: 'linear-gradient(to top, rgba(5,5,5,0.9) 30%, rgba(5,5,5,0.2) 70%, transparent)' }}>
                <div className="transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                  <p className="text-ash text-xs mb-1" style={{ fontFamily: 'DM Mono, monospace' }}>{p.category}</p>
                  <p className="font-display font-bold text-bone text-lg sm:text-xl tracking-tight">{p.nameKey}</p>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-volt font-semibold text-sm" style={{ fontFamily: 'DM Mono, monospace' }}>{p.price}</span>
                    <span className="text-xs text-ink bg-volt px-2 sm:px-3 py-1 sm:py-1.5 tracking-wide uppercase">{t.grid_shop}</span>
                  </div>
                </div>
              </div>

              <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-4 sm:px-5 py-3 sm:py-4 group-hover:opacity-0 transition-opacity duration-300" style={{ background: 'linear-gradient(to top, rgba(5,5,5,0.7), transparent)' }}>
                <p className="font-display font-bold text-bone text-base sm:text-lg tracking-tight">{p.nameKey}</p>
                <span className="text-ash text-xs sm:text-sm" style={{ fontFamily: 'DM Mono, monospace' }}>{p.price}</span>
              </div>

              {p.tag && (
                <div className="absolute top-3 sm:top-4 left-3 sm:left-4 px-2 sm:px-3 py-1 text-xs tracking-[0.12em] uppercase" style={{ fontFamily: 'DM Mono, monospace', background: p.accent, color: '#050505' }}>
                  {p.tag}
                </div>
              )}
