  ...
  return (
    <div className="min-h-screen bg-ink flex flex-col">
      <div className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/5">
        <Link to="/"><img src={logo} alt="Drippy" className="h-22 sm:h-26 w-auto object-contain" /></Link>
          ...
          <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
            <div className="flex flex-col gap-1.5">
              ...
              <input id="email" type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com"
                className={`w-full px-3.5 py-2.5 bg-white/[0.03] border rounded-xl text-sm text-bone placeholder:text-ash/40 outline-none transition-all focus:ring-2 focus:ring-volt/20 focus:border-volt/50 ${errors.email ? 'border-rose-500/60' : 'border-white/10 hover:border-white/20'}`}
              />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                <input id="password" type={showPassword ? 'text' : 'password'} autoComplete="current-password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••"
                  className={`w-full px-3.5 py-2.5 pr-10 bg-white/[0.03] border rounded-xl text-sm text-bone placeholder:text-ash/40 outline-none transition-all focus:ring-2 focus:ring-volt/20 focus:border-volt/50 ${errors.password ? 'border-rose-500/60' : 'border-white/10 hover:border-white/20'}`}
                />
                <button type="button" onClick={() => setShowPassword((v) => !v)} className="absolute inset-y-0 right-3.5 flex items-center text-ash hover:text-bone transition-colors" tabIndex={-1}>
            ...
            <button type="submit" disabled={isLoading} className="mt-1 w-full flex items-center justify-center gap-2 px-5 py-3 bg-volt text-ink text-xs font-bold uppercase tracking-[0.12em] rounded-xl hover:bg-bone transition-all disabled:opacity-60 disabled:cursor-not-allowed cursor-pointer" style={{ fontFamily: 'DM Mono, monospace' }}>
              {isLoading ? (<><Loader2 className="w-3.5 h-3.5 animate-spin" />{t.login_submitting}</>) : (<><LogIn className="w-3.5 h-3.5" />{t.login_submit}</>)}
            </button>
          </form>
